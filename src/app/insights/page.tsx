...
export default function InsightsPage() {
  ...
  const summaryInsight = useMemo(() => {
    if (!insights || insights.length === 0) return "Processing AI intelligence data...";
    ...
    let rec = "";
    if (pendingCount > 0) {
      rec = `Currently there are ${pendingCount} AI recommendations waiting for implementation. If fully executed, it has the potential to add savings/revenue up to Rp ${totalPotential.toLocaleString('en-US')}.`;
    } else {
      rec = "All AI computation suggestions have been successfully and efficiently executed.";
    }
    return `The AI system generated ${insights.length} insights based on operational data. ${rec}`;
  }, [insights]);
  ...
  const columns = [
    ...
    { title: 'Potential Revenue', dataIndex: 'potential_revenue', key: 'revenue', align: 'right' as const, render: (v: number) => <Text strong>Rp {v?.toLocaleString('en-US')}</Text> },
  ...
  return (
    <MainLayout title="AI Insights" subtitle="Smart recommendations based on artificial intelligence from CALF AI">
      {isLoading ? (
          ...
          <SectionContainer style={{ marginBottom: 24, padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '24px 24px 0 24px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
              <Space align="center">
                ...
                <Tooltip title="Suggestions, anomaly detection, and optimization recommendations automatically generated by AI.">
            ...
            <Table 
              columns={columns} 
              dataSource={insights} 
              pagination={{ pageSize: 5, showSizeChanger: true, showTotal: (t) => `Total ${t} insights` }} 
              size="middle" 
              rowKey="id" 
              scroll={{ x: 1000 }} 
