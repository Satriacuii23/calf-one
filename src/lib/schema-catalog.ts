...
export const BILINGUAL_FIELD_SYNONYMS: Record<string, string[]> = {
  // Products
  product_code: ['product_code', 'code', 'sku', 'kode_produk', 'kode_barang', 'kode', 'id_produk', 'id_barang'],
  product_name: ['product_name', 'name', 'item_name', 'nama_produk', 'nama_barang', 'menu', 'nama_menu', 'item', 'barang'],
  category_name: ['category_name', 'category', 'kategori', 'jenis', 'golongan', 'kelompok'],
  unit_price: ['unit_price', 'price', 'selling_price', 'harga', 'harga_satuan', 'harga_jual', 'nominal'],
  is_active: ['is_active', 'active', 'status', 'aktif', 'tersedia'],
  ...
  phone_number: ['phone_number', 'phone', 'whatsapp', 'no_hp', 'telepon', 'no_telp', 'nomor_hp'],
  email: ['email', 'email_address', 'surel', 'alamat_email'],
...
export function canonicalizeKey(rawKey: string): string {
  ...
  for (const [canonical, syns] of Object.entries(BILINGUAL_FIELD_SYNONYMS)) {
    if (syns.includes(normalized) || syns.some(s => normalized === s || normalized.includes(s))) {
      return canonical;
    }
  }
  return normalized;
